  ...
  return (
    <div className="relative w-full h-[600px] flex items-center justify-center overflow-visible">
      ...
      <div className="relative z-10 w-[280px] h-[340px] md:w-[320px] md:h-[400px]">
        ...
        >
          ...
          <div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl bg-white/60 dark:bg-white/5 backdrop-blur-md border border-white/40 dark:border-white/10 transition-colors shadow-lg dark:shadow-none">
             <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black text-zinc-600 dark:text-zinc-400 uppercase tracking-widest transition-colors">Digital Architect</span>
             </div>
             <div className="text-sm font-bold text-zinc-900 dark:text-white transition-colors">Focus on Precision</div>
          </div>
        </motion.div>

        {/* Orbiting UI Badges */}
        <OrbitingBadge 
          icon={<div className="w-4 h-4 rounded-sm border-2 border-zinc-500 dark:border-white transition-colors" />} 
          label="UI/UX" 
          angle={45} 
          radius={180} 
          delay={0}
        />
        <OrbitingBadge 
          icon={<div className="w-4 h-4 bg-lime-500 rounded-full" />} 
          label="React" 
          angle={180} 
          radius={220} 
          delay={1}
        />
        <OrbitingBadge 
          icon={<div className="w-4 h-4 border border-zinc-400 rotate-45" />} 
          label="Strategy" 
          angle={300} 
          radius={200} 
          delay={2}
