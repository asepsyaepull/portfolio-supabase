...
export default function HowIBuildSection() {
  ...
  return (
    <section className="overflow-hidden bg-[var(--canvas)] py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-5" ref={headRef}>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="omd-frame-label mb-3">PROCESS.TIMELINE</p>
            <h2 className="font-display text-[clamp(38px,6vw,72px)] font-semibold leading-[1.02] tracking-tight text-[var(--ink)]">
              FROM BRIEF
              <br />
              TO <span className="text-[var(--brand)]">LIVE.</span>
            </h2>
            <p className="mt-4 max-w-[52ch] text-[15px] md:text-base text-[var(--ink-soft)]">
              Timeline tipikal satu project. Geser buat lihat tiap fase —
              termasuk kapan lo bakal lihat draft pertama.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-full border border-[var(--line-2)] bg-white text-[var(--ink)] transition hover:border-[var(--brand)] hover:text-[var(--brand)] disabled:opacity-30"
            >
        ...
        >
            ...
            return (
              <article
                ...
                className={`omd-card relative flex w-[85vw] shrink-0 snap-start flex-col p-6 transition-all duration-300 sm:w-[46%] lg:w-[31%] ${
                  isActive
                    ? "ring-2 ring-[var(--brand)] ring-offset-2 ring-offset-[var(--canvas)]"
                    : ""
                }`}
              >
                {/* header */}
                <div className="mb-5 flex items-center justify-between">
                  ...
                  <span className="font-mono text-[11px] font-bold tracking-widest text-[var(--ink-faint)]">
                    {ph.duration}
                  </span>
                </div>
                ...
                <h3 className="mt-1 font-display text-xl font-semibold text-[var(--ink)] md:text-2xl">
                  {ph.title}
                </h3>
                ...
                {/* progress bar */}
                <div className="mt-5">
                  <div className="mb-1.5 flex justify-between font-mono text-[10px] uppercase tracking-wide text-[var(--ink-faint)]">
                    <span>progress</span>
                    <span>{ph.progress}%</span>
                  </div>
                  <div className="h-1.5 overflow-hidden rounded-full bg-[var(--grid)]">
                    <motion.div
                      initial={{ width: 0 }}
          ...
          <div className="flex w-[60vw] shrink-0 snap-start items-center justify-center sm:w-[40%] lg:w-[25%]">
            <p className="text-center font-mono text-xs uppercase leading-relaxed tracking-widest text-[var(--ink-faint)]">
              — dan ulangi untuk
              <br />
              <span className="text-[var(--brand)]">project berikutnya.</span>
            </p>
          </div>
        </motion.div>
      </div>
