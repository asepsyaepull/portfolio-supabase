import { AboutPageDictionary } from "../types";

export const aboutPageEn: AboutPageDictionary = {
  badge: "/ PROFESSIONAL PROFILE",
  headlineMain: "UI/UX Design",
  headlineSub: "& Developer",
  bio1Prefix: "I'm ",
  bio1Name: "Asep Syaepul Rohman",
  bio1Suffix:
    ", a UI/UX Designer & Frontend Developer with 7+ years of experience designing and building production-scale digital product ecosystems.",
  bio2: "Bridging the gap between high-fidelity design and frontend development. My approach combines targeted user research with clean, structured code implementation, ensuring every interface component fulfills a real functional purpose and delivers optimal performance.",
  skillsTitle: "Core Disciplines & Capabilities",
  designTitle: "UI/UX & Product Design",
  designSkills: [
    { label: "User Research & Usability Testing", percent: 95 },
    { label: "Interactive Flows & High-Fidelity Prototyping", percent: 90 },
    { label: "Information Architecture & User Journeys", percent: 85 },
    { label: "Design Systems & Token Architecture", percent: 95 },
  ],
  devTitle: "Frontend Engineering",
  devSkills: [
    { label: "React.js & Next.js Ecosystem", percent: 90 },
    { label: "TypeScript (Type-Safe Architecture)", percent: 85 },
    { label: "Tailwind CSS & Modular Design", percent: 95 },
    { label: "Core Web Vitals & Runtime Optimization", percent: 80 },
  ],
  foundationsTitle: "Education & Certifications",
  education: [
    {
      school: "Binus University",
      degree: "Bachelor of Computer Science (S.Kom) — Information Systems",
      year: "2018 - 2020",
      gpa: "GPA: 3.20",
    },
    {
      school: "IPB University",
      degree: "Associate Degree (D3) — Informatics Management",
      year: "2014 - 2017",
      gpa: "GPA: 2.81",
    },
    {
      school: "Dibimbing.id",
      degree: "Front-End Web Development Bootcamp (Distinction)",
      year: "2024 - 2025",
      gpa: "Final Score: 96.93",
    },
  ],
  journeyTitle: "Professional Track Record",
  statusBadge: "Available for Strategic Contracts & Full-time Roles",
  experienceYears: "7+ Years Experience",
  location: "Jakarta, Indonesia",
  rolesHeadline: "UI/UX Designer & Frontend Developer",
  viewProjectsBtn: "View All Projects",
  contactBtn: "Contact Me",
};
